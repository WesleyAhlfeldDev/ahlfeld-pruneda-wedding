...
export function emptyState() {
  return {
    ...
    media: [],
    decision: { venueId: null, packageId: null, decidedAt: null },
    travelInfo: {
      venueSameAsHotel: false,
      allInclusive: false,
      adultsOnly: false,
      international: false,
      airportShuttleIncluded: false,
      roomBlockAvailable: false,
      resortBasics: {}, gettingThere: {}, roomInfo: {}, hotelDining: {},
      amenities: {}, spa: {}, forFamilies: {}, adultOnly: {},
      practicalInfo: {}, excursions: {}, emergency: {},
    },
    guestCount: '',
    weddingDate: '',
  }
...
// decision is embedded in estimate JSON to avoid requiring a schema change
function stateToRow(state) {
  return {
    ...
    estimate: {
      ...(state.estimate ?? emptyState().estimate),
      _decision: state.decision ?? emptyState().decision,
      _travelInfo: state.travelInfo ?? emptyState().travelInfo,
      _media: state.media ?? [],
    },
...
function rowToState(row) {
  const { _decision, _travelInfo, _media, ...estimateFields } = row.estimate ?? {}
  return {
    venues: row.venues ?? [],
    notes: row.notes ?? [],
    todos: row.todos ?? [],
    ...
    estimate: { ...DEFAULT_ESTIMATE, ...estimateFields },
    decision: _decision ?? emptyState().decision,
    travelInfo: _travelInfo ?? emptyState().travelInfo,
    media: _media ?? [],
