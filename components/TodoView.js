...
const PRIORITIES = [
  ...
  { id: 'low', label: 'Low', color: 'bg-sage-100 text-sage-600 border-sage-200' },
]
...
function TodoItem({ todo, onUpdate, onDelete }) {
  ...
  if (editing) {
    return (
      <div className="bg-forest-500 rounded-xl border-2 border-plum-300 p-4 space-y-3">
        <input
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs text-moon-300 font-sans">Priority:</span>
          {PRIORITIES.map(p => (
            <button key={p.id} onClick={() => setDraft(d => ({ ...d, priority: p.id }))}
              className={`text-xs font-sans px-2.5 py-1 rounded-full border transition-all ${draft.priority === p.id ? p.color + ' font-semibold' : 'bg-forest-500 text-moon-300 border-plum-600 hover:border-plum-400'}`}>
              {p.label}
            </button>
          ))}
          <span className="text-xs text-moon-300 font-sans ml-2">Category:</span>
          <div className="relative">
            <select
              className="text-xs font-sans px-2.5 py-1 rounded-full border border-plum-600 bg-forest-500 text-plum-200 appearance-none pr-6 cursor-pointer"
            ...
            <ChevronDown className="absolute right-1.5 top-1/2 -translate-y-1/2 w-3 h-3 text-white0 pointer-events-none" />
          </div>
        </div>
        <textarea
  ...
  return (
    <div className={`flex items-start gap-3 p-4 rounded-xl border transition-all group ${
      todo.completed ? 'bg-forest-600 border-plum-700/50 opacity-60' : 'bg-forest-500 border-plum-700/50 hover:border-plum-600 hover:shadow-sm'
    }`}>
      {/* Checkbox */}
      <button
        onClick={toggle}
      ...
      <div className="flex-1 min-w-0">
        <p className={`font-sans text-sm ${todo.completed ? 'line-through text-moon-300' : 'text-plum-50'}`}>
          {todo.text}
        </p>
        {todo.notes && !todo.completed && (
          <p className="text-xs text-moon-300 mt-0.5 font-sans">{todo.notes}</p>
        )}
        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
          ...
          {todo.category && todo.category !== 'other' && (
            <span className="text-xs text-moon-300 font-sans">
      ...
      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
        ...
        <button onClick={() => onDelete(todo.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-white0 hover:text-red-400 transition-colors">
  ...
  return (
    <div className="space-y-5 max-w-2xl">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-forest-500 rounded-2xl border border-plum-700/50 p-3 text-center shadow-sm">
          <p className="font-serif text-2xl font-bold text-plum-50">{active.length}</p>
          <p className="text-xs text-moon-300 font-sans mt-0.5">Remaining</p>
        </div>
        <div className="bg-forest-500 rounded-2xl border border-plum-700/50 p-3 text-center shadow-sm">
          <p className="font-serif text-2xl font-bold text-sage-600">{completed.length}</p>
          <p className="text-xs text-moon-300 font-sans mt-0.5">Done</p>
        </div>
        <div className="bg-forest-500 rounded-2xl border border-plum-700/50 p-3 text-center shadow-sm">
          <p className="font-serif text-2xl font-bold text-plum-50">
            {todos.length > 0 ? Math.round((completed.length / todos.length) * 100) : 0}%
          </p>
          <p className="text-xs text-moon-300 font-sans mt-0.5">Complete</p>
        </div>
      </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {PRIORITIES.map(p => (
              <button key={p.id} onClick={() => setNewPriority(p.id)}
                className={`text-xs font-sans px-2.5 py-1 rounded-full border transition-all ${newPriority === p.id ? p.color + ' font-semibold' : 'bg-forest-500 text-moon-300 border-plum-600 hover:border-plum-400'}`}>
                {p.label}
              </button>
            ))}
            <span className="text-xs text-moon-300 font-sans ml-2">Category:</span>
      ...
      {filtered.length > 0 ? (
        ...
        <div className="card p-12 text-center">
          ...
          <h3 className="font-serif text-xl text-plum-50 mb-1">
            {active.length === 0 ? 'All caught up!' : 'No tasks in this category'}
          </h3>
          <p className="text-sm text-moon-300 font-sans mb-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <button
              ...
              className="flex items-center gap-1.5 text-sm text-moon-300 hover:text-white font-sans transition-colors"
            >
              <ChevronDown className={`w-4 h-4 transition-transform ${showCompleted ? 'rotate-180' : ''}`} />
              {completed.length} completed
            </button>
            <button onClick={clearCompleted} className="text-xs text-white0 hover:text-red-400 font-sans transition-colors">
              Clear all
            </button>
          </div>
          {showCompleted && (
